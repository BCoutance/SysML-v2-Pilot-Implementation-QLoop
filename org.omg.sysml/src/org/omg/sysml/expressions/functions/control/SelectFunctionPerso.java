...
public class SelectFunctionPerso implements LibraryFunction {
	...
	public EList<Element> invoke(InvocationExpression invocation, Element target, ExpressionEvaluator evaluator) {
		...
		if (collection != null) {				
			Expression selector = (Expression) ((FeatureReferenceExpression)invocation.getArgument().get(1)).getReferent();	
			// if the selector is an already defined function, we use it instead of creating a new function
			// Evaluation is the most generic type of function, if we catch it it means we have a generic expression 
			Function func = null;
			Boolean isGenericExpr = selector.getType().get(0).getQualifiedName().equals("Performances::Evaluation");
			if (!isGenericExpr) func = (Function) selector.getType().get(0);
			else func = createFunction(selector);
					
			for (Element element : collection) {
				if (element instanceof Feature elementFeature) {					
					InvocationExpression invoc = createInvocation(func, elementFeature);
					// Evaluate this invocation to add the element to the selected results if it matches the condition
					EList<Element> values = evaluator.evaluateInvocation(invoc, target); 
					if (((LiteralBoolean) values.get(0)).isValue())	result.add(element);
				}
			}
			if (isGenericExpr) resetSelectorFeatures(selector, func);
			return result;
		}
		return EvaluationUtil.singletonList(invocation);		
	}

	private Function createFunction(Expression selector) {
		// Build a new function based on the selector expression that borrows its FeaturesMemberships 
		// Since the selector is not elsewhere defined, the selector will not be accessed before another call of this select invoke
		// The FeaturesMemberships are returned at the end of the invoke call
		Function func = SysMLFactory.eINSTANCE.createFunction();

		FeatureMembership fm0 = selector.getOwnedFeatureMembership().get(0);
		...
		FeatureMembership fm1 = selector.getOwnedFeatureMembership().get(1);
		...
		selector.eResource().getContents().add(func);
	...
	private void resetSelectorFeatures(Expression selector, Function func) {
		// return the FeaturesMemberships to the selector
		EList<FeatureMembership> selectorfm = selector.getOwnedFeatureMembership();
		EList<FeatureMembership> funcfm = func.getOwnedFeatureMembership();	
		if (selectorfm != null && funcfm != null && selectorfm.size()==funcfm.size()) {
			for (int i = 0; i < selectorfm.size(); i++) {
				selectorfm.get(i).setOwnedMemberFeature(funcfm.get(i).getOwnedMemberFeature());
			}
		}
		selector.eResource().getContents().remove(func);
	}	
}